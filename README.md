...
### Manage your profile and publications locally

This repository is a Jekyll academic-homepage template. The displayed name, portrait, biography, location, email address, and publications are edited in local YAML files; the homepage does not read the GitHub owner's profile or display a GitHub repository list.

Add your research interests, publication record, academic biography, and optional blog posts to personalize the site.

The site uses [Jekyll](https://jekyllrb.com/docs/) for building and can be hosted with [GitHub Pages](https://pages.github.com/).
...
**The fastest approach** is to rename your repository `username.github.io`, where `username` is your GitHub username (or organization name). Configure GitHub Pages to publish from your chosen branch (this repository currently uses `main`), then pushes to that branch will update the site.
...
Edit the `profile` dictionary in `_config.yml`. The main biography comes from `bio` and supports Markdown, including links such as `[City University of Hong Kong](https://www.cityu.edu.hk/)`. The optional short text below the portrait comes from `sidebar_bio`. Put a square portrait in `assets/images/` and set `avatar` to a path such as `/assets/images/avatar.jpg`. Set `email` to the plain email address only (without Markdown or `mailto:`); the page displays it as “My Email Address” and creates the `mailto:` link automatically. Paste the full profile URL into `google_scholar` to activate the Google Scholar link. Empty optional values are not rendered, except the empty Scholar slot, which remains visible as a non-clickable label until its URL is filled.

#### Publications

Edit `_data/publications.yml`. Copy a full entry for each additional publication. Entries are grouped by year and displayed newest first according to `sort_date`. To mark co-primary authors, add `co_primary: true` below each corresponding author name; the page will add a superscript `*` and the equal-contribution note automatically.

#### News

Edit `_data/news.yml`. Each entry with `show: true` is rendered as one bullet point between About Me and Selected Publications. The optional `date` appears in bold, and `text` supports Markdown links, bold text, and italics. Items follow their order in the YAML file, so place the newest item first.
...
- `masthead.html`: Displays the locally configured avatar, name, bio, email, and location.
- `post-card.html`: A compact, summarized presentation of a blog post, re-used to display a listing of your latest blog posts.
- `publications.html`: Groups and displays the entries in `_data/publications.yml`.
...
- **home**: Used by your `index.html` homepage to display publications, interests, and (optionally) blog posts.
